 ...
 *    through the bulk date sync (e.g. leagues outside the ingest rank-gate),
 *    so their result never arrives on its own. We fetch the result directly
 *    by fixture id from API-Sports, update status/scores, and settle. Scoped
 *    to fixtures with real money on them, so upstream volume stays tiny.
 *
 ...
 * their league falls outside the ingest rank-gate, so we pull each one's
 * result directly by fixture id and settle.
 */
