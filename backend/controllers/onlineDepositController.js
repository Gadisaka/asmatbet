...
function verifySummaryFromResponse(method, data) {
  ...
  if (m === "telebirr") {
    const d = telebirrReceiptFromVerifyBody(data);
    return {
...
function remotePayloadForMethod(method, parts) {
  ...
  if (m === "cbe") {
    ...
    const suffix = sanitizePaySegment(parts.accountSuffix);
...
export async function verifyOnlineDeposit(req, res) {
  try {
    ...
    if (methodRaw === "cbe") {
      const newTok = extractNewCbeReceiptToken(parts.reference);
      if (newTok) {
        parts.reference = newTok;
        parts.accountSuffix = "";
      } else {
        const combined = String(parts.reference ?? "")
          .trim()
          .match(/^(FT[A-Z0-9]{10})(\d{8})$/i);
        if (combined) {
          parts.reference = combined[1].toUpperCase();
          parts.accountSuffix = combined[2];
        }
      }
    }
    ...
    if (methodRaw === "cbe") {
      const cbeRef = sanitizePaySegment(parts.reference);
      if (
        isLegacyCbeFtReference(cbeRef) &&
        !sanitizePaySegment(parts.accountSuffix)
      ) {
        return res.status(400).json({
          message: smsText
            ? "Could not read account suffix from SMS."
            : "accountSuffix is required for legacy CBE receipts (or paste full SMS).",
        });
      }
    }
