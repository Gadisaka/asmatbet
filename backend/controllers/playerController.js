...
import { refundPlayerOnlineBetInTx } from "../services/ticketCancelRefunds.js";
import { ticketWinningsTaxApiFields } from "../lib/winningsTax.js";
  ...
  try {
    ...
    try {
      outcomeKind = await prisma.$transaction(async (tx) => {
        ...
        await refundPlayerOnlineBetInTx(tx, locked);
...
function mapPlayerTicket(ticket) {
  ...
  const taxFields = ticketWinningsTaxApiFields(ticket);
