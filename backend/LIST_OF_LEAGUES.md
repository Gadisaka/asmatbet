...
- **Priority ranks:** `Config/leagueRanks.js` (1 = highest). Ranks 1–8 = preferred top leagues; **9–10 = `PRODUCT_PRIORITY_LEAGUE_IDS` (Ethiopia)**; 11+ = legacy allowlist seed; remainder = catalogue tail.
- **Ingestion:** each bulk sync stores fixtures only for the **top 200 active leagues by rank** (`INGEST_ACTIVE_CAP`).
- **Sidebar:** active leagues only — top section (preferred + product priority when active) plus up to `SIDEBAR_ACTIVE_CAP` regional leagues under country tabs.
