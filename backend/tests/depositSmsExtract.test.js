...
import {
  extractNewCbeReceiptToken,
  extractOnlineDepositFromSms,
  isNewCbeReceiptToken,
} from "../lib/depositSmsExtract.js";
...
const CBE_NEW_SMS =
  "Dear Gadisa, You have transferred ETB 50.00 to Tewachew Adimasu. Thank you for Banking with CBE! https://mbreciept.cbe.com.et/fHCxyU3pPQIUBir8hu";

test("extractOnlineDepositFromSms CBE parses new mbreciept token URL", () => {
  const r = extractOnlineDepositFromSms("cbe", CBE_NEW_SMS);
  assert.equal(r.ok, true);
  if (r.ok) {
    assert.equal(r.reference, "fHCxyU3pPQIUBir8hu");
    assert.equal(r.accountSuffix, "");
  }
});

test("extractOnlineDepositFromSms CBE accepts a bare new receipt token", () => {
  const r = extractOnlineDepositFromSms("cbe", "fHCxyU3pPQIUBir8hu");
  assert.equal(r.ok, true);
  if (r.ok) {
    assert.equal(r.reference, "fHCxyU3pPQIUBir8hu");
    assert.equal(r.accountSuffix, "");
...
test("extractNewCbeReceiptToken reads URL or bare token", () => {
  assert.equal(
    extractNewCbeReceiptToken(
      "https://mbreciept.cbe.com.et/fHCxyU3pPQIUBir8hu",
    ),
    "fHCxyU3pPQIUBir8hu",
  );
  assert.equal(isNewCbeReceiptToken("FT26095YRWP5"), false);
});
...
test("extractOnlineDepositFromSms telebirr parses Transaction ID label", () => {
  const r = extractOnlineDepositFromSms(
    "telebirr",
    "You paid 20 Birr. Transaction ID: CE2513001X",
  );
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.reference, "CE2513001X");
});

test("extractOnlineDepositFromSms telebirr parses Amharic transaction number", () => {
  const r = extractOnlineDepositFromSms(
    "telebirr",
    "የግብይት ቁጥርዎ DD23HGV3T7 ነው።",
  );
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.reference, "DD23HGV3T7");
});

test("extractOnlineDepositFromSms telebirr accepts a bare 10-char id", () => {
  const r = extractOnlineDepositFromSms("telebirr", "DD23HGV3T7");
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.reference, "DD23HGV3T7");
});
