...
test("buildOnlinePayLedgerReference is stable and unique per channel", () => {
  ...
  assert.equal(
    buildOnlinePayLedgerReference("cbe", {
      reference: "fHCxyU3pPQIUBir8hu",
    }),
    "online-pay:cbe:fHCxyU3pPQIUBir8hu",
  );
  assert.equal(
...
test("parseVerifiedAmountEtb uses Telebirr settled amount, not total paid", () => {
  assert.equal(
    parseVerifiedAmountEtb("telebirr", {
      success: true,
      data: {
        settledAmount: "30.00 Birr",
        totalPaidAmount: "31.00 Birr",
      },
    }),
    30,
  );
  assert.equal(
    parseVerifiedAmountEtb("telebirr", {
      success: true,
      data: { settledAmount: "1,250.00 Birr" },
    }),
    1250,
  );
  assert.equal(
