 ...
 *
 * Run inside the staging backend container:
 *   docker compose -p michubet-staging -f docker-compose.staging.yml \
 *     exec backend node scripts/seedInoutTestPlayer.mjs
 *
...
async function run() {
  ...
  if (!wallet) {
    wallet = await prisma.wallet.create({
      data: { user_id: user.id, wallet_type: "PLAYER", balance: BALANCE },
    });
