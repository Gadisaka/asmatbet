...
router.get("/odds/live", async (_req, res) => {
  try {
        if (isOddspapiPublic()) {
      const oddspapiBook = await resolveOddspapiBookmaker();
      const rows = await prisma.fixture.findMany({
        where: {
          status: { in: ["LIVE", "HT"] },
          ...notOddspapiWhere(),
        ...
        take: LIVE_FIXTURES_LIMIT,
        include: {
          markets: buildMarketsInclude(oddspapiBook?.id ?? null),
        },
        orderBy: { start_time: "asc" },
      });
...
router.get("/odds/:apiFixtureId", async (req, res) => {
  try {
    ...
    const preferred = isOddspapiPublic()
      ? await resolveOddspapiBookmaker()
      : await getPreferredBookmakerRecord();
