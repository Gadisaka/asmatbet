# InOut Games Integration — AsmatBet

Complete reference for the **seamless-wallet** integration with [InOut Games](https://inout.games). AsmatBet holds player balances; InOut calls our webhook for `init`, `bet`, `withdraw`, and `rollback`. Games launch in an iframe via InOut’s launch URL.

**Production callback URL:** `https://api.asmatbet.com/api/integrations/inout/webhook`  
**Currency:** ETB (Ethiopian Birr)  
**Brand:** AsmatBet (InOut operator alias remains `michotbet-prod` until InOut reissues)  

> This document covers the **production** backend (`api.asmatbet.com`). We use one backend for both InOut certification and live play — no separate staging stack on the VPS.
...
| Topic | Choice |
|--------|--------|
| Wallet model | Seamless — AsmatBet debits/credits ETB on webhook |
...
Maps InOut `authToken` → AsmatBet user for webhooks.
...
```
https://api.golden-eggs.games/api/launch?
  gameMode={mode}
...
curl -s -o /dev/null -w "%{http_code}\n" \
  -X POST https://api.asmatbet.com/api/integrations/inout/webhook
```
...
```bash
cd ~/asmatbet/admin && npm run build
```
...
BASE_URL="https://api.asmatbet.com" \
...
export const INOUT_LAUNCH_BASE_URL =
  process.env.INOUT_LAUNCH_BASE_URL || "https://api.golden-eggs.games/api/launch";
...
```
Webhook:     POST https://api.asmatbet.com/api/integrations/inout/webhook
Player lobby: https://asmatbet.com/casino
