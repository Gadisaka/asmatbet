...
| **`API_URL`**    | Must point at the same backend as production (e.g. `https://api.asmatbet.com/api`) — baked in at **build time** |
...
```js
export const API_URL = "https://api.asmatbet.com/api";
```
...
```bash
sudo mkdir -p /var/www/asmatbet-admin
rsync -avz --delete admin/dist/ user@your-vps:/var/www/asmatbet-admin/
```
...
sudo rsync -av --delete dist/ /var/www/asmatbet-admin/
```
...
Example:** `deploy/nginx/asmatbet-admin.example.conf`**

Install:

```bash
sudo cp /path/to/repo/deploy/nginx/asmatbet-admin.example.conf /etc/nginx/sites-available/asmatbet-admin.conf
sudo nano /etc/nginx/sites-available/asmatbet-admin.conf
sudo ln -sf /etc/nginx/sites-available/asmatbet-admin.conf /etc/nginx/sites-enabled/
...
| `deploy/nginx/asmatbet-admin.example.conf` | nginx template                             |
