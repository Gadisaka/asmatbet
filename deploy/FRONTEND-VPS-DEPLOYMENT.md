...
| **API**          | HTTPS origin like `https://api.asmatbet.com` — injected at **build time**  |
...
- A hostname for the site (examples: **`https://asmatbet.com`**, **`https://bet.your-domain.com`**).
...
VITE_API_URL=https://api.asmatbet.com
...
```bash
sudo mkdir -p /var/www/asmatbet-frontend
# from your dev machine or CI:
rsync -avz --delete frontend/dist/ user@your-vps:/var/www/asmatbet-frontend/
```
...
sudo rsync -av --delete dist/ /var/www/asmatbet-frontend/
```
...
Example file in the repo: **`deploy/nginx/asmatbet-frontend.example.conf`**.

Install (replace domain and root path):

```bash
sudo cp /path/to/repo/deploy/nginx/asmatbet-frontend.example.conf /etc/nginx/sites-available/asmatbet-frontend.conf
sudo nano /etc/nginx/sites-available/asmatbet-frontend.conf   # server_name, root
sudo ln -sf /etc/nginx/sites-available/asmatbet-frontend.conf /etc/nginx/sites-enabled/
...
| `deploy/nginx/asmatbet-frontend.example.conf` | nginx template                   |
