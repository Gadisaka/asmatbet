...
2. **nginx**: Example site `deploy/nginx/api.asmatbet.com.conf` proxies **`api.asmatbet.com`** → `127.0.0.1:3001`, with a **`/nginx-health`** route answered by nginx (not Node).
...
- A domain/subdomain pointing at the server (example: **`api.asmatbet.com`** → server IPv4 A record).
...
```bash
sudo cp /path/to/your/repo/deploy/nginx/api.asmatbet.com.conf /etc/nginx/sites-available/api.asmatbet.com.conf
sudo ln -sf /etc/nginx/sites-available/api.asmatbet.com.conf /etc/nginx/sites-enabled/
...
sudo certbot --nginx -d api.asmatbet.com
```
...
Point your API base URL to **`https://api.asmatbet.com`** (`VITE_*` or equivalent). The backend historically used permissive CORS (`origin: "*"`); tighten for production when you pin allowed origins.
...
```bash
curl -sS https://api.asmatbet.com/health
```

HTTP only (before TLS):

```bash
curl -sS http://api.asmatbet.com/health
```

### 9.2 nginx only (does not hit Node)

```bash
curl -sS https://api.asmatbet.com/nginx-health
```

Expect **`ok`** (or whatever your `api.asmatbet.com.conf` returns).  
...
```bash
curl -sS -o /dev/null -w "%{http_code}\n" https://api.asmatbet.com/api/football/fixtures/today
```
...
| `deploy/nginx/api.asmatbet.com.conf` | Example nginx reverse proxy |
