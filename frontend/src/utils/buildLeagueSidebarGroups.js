...
/**
 * @param {Array<{ id: string, label?: string, leagueLogo?: string|null, countryFlag?: string|null, pinned?: boolean, rank?: number, section?: string, count?: number }>} catalogItems
 *   Ordered list from GET /api/football/sidebar-leagues (empty = fallback to counts-only).
 *   Top-section leagues are included so country/region totals count their coming games.
 * @param {Map<string, number>} counts — league display key → match count (catalog upcoming on Home).
