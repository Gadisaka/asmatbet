 ...
 * Escape.
 *
 * @param {{ url?: string, title?: string, poster?: string, onClose: () => void }} props
 */
function GameFrame({ url, title, poster, onClose }) {
  useEffect(() => {
    const onKey = (e) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[300] flex flex-col bg-black">
      ...
      {poster ? (
        <div className="flex flex-1 items-center justify-center bg-[#050505] p-6">
          <img
            src={poster}
            alt=""
            className="max-h-[78vh] w-full max-w-sm rounded-3xl object-cover shadow-[0_24px_80px_rgba(0,0,0,0.45)]"
          />
        </div>
      ) : (
        <iframe
