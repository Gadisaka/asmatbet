...
function MobileMenu({ open, onClose }) {
  ...
  return (
    <>
      ...
      >
        <div className="pointer-events-none absolute -left-16 top-24 h-40 w-48 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(143,207,74,0.16),transparent_70%)] blur-2xl" />
        ...
        <div className="relative flex h-14 shrink-0 items-center justify-between px-4 shadow-[0_12px_28px_-16px_rgba(0,0,0,0.6)]">
          <div
            ...
            aria-hidden
          />
          <p className="m-0 text-xs font-extrabold uppercase tracking-[0.18em] text-(--sb-on-accent)">
            {t("menu.accountTitle")}
          </p>
          <button
            type="button"
            onClick={onClose}
            className="relative z-10 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border-0 bg-[#0a0a0a]/18 text-(--sb-on-accent) transition-transform hover:scale-105 active:scale-95"
        ...
        <div className="relative flex-1 overflow-y-auto px-3 pb-8 pt-4">
          ...
          <button
            ...
            className="mb-4 flex w-full cursor-pointer items-center gap-3 rounded-xl border-0 bg-[#0a0a0a]/40 px-4 py-3.5 text-left text-sm font-extrabold text-[#ffffff]  transition-all duration-200 hover:bg-(--sb-accent-surface)/35 hover:ring-(--sb-accent-fill)/35"
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#111111] text-[rgba(255,255,255,0.72)] ">
