...
function DesktopUserSidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      />
      <div
        className="animate-deposit-panel fixed right-3 top-[3.75rem] z-[90] w-[min(calc(100%-1.5rem),17.5rem)] origin-top-right overflow-hidden rounded-[1.35rem] bg-gradient-to-br from-[#111111]/98 via-[#111111]/98 to-[#000000]/98 py-2 shadow-[0_24px_60px_-12px_rgba(0,0,0,0.65)]  backdrop-blur-md transition-shadow duration-300 max-lg:hidden"
        role="dialog"
        aria-label={t("common.accountMenu")}
      >
        <div className="pointer-events-none absolute inset-x-0 -top-12 h-28 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(143,207,74,0.18),transparent_70%)] blur-2xl" />
        <div className="relative px-2 pt-1 pb-2">
              ...
              return (
                ...
                >
                  {item.icon ? (
                    ...
                    <span className="h-8 w-8 shrink-0 rounded-lg bg-[#0a0a0a]/40 ring-1 ring-[#8FCF4A]/30" />
                  )}
          ...
          <button
            ...
            className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-bold text-[#ffffff] transition-all duration-200 hover:translate-x-0.5 hover:bg-[#0a0a0a]/75"
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#0a0a0a]/80 text-[rgba(255,255,255,0.72)]">
              <AppIcon name="palette" size={16} />
            </span>
            <span className="min-w-0 flex-1 leading-tight">{t("menu.theme")}</span>
          </button>
        </div>
        <div className="relative mx-2 mb-2 h-px bg-(--sb-accent-fill)/60" />
        <button
          type="button"
