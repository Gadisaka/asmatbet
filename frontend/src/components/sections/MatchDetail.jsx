...
function CompactMatchRow({ match, isSelected, onClick }) {
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-b-[#2f3047] px-2.5 py-1 text-xs text-[#cfd2e4]">
...
function OddsCategory({
  ...
  return (
    <div className="border-b border-b-[#2f3047]">
      <button
        type="button"
        onClick={() => setOpen((p) => !p)}
  ...
  return (
    <div className="grid gap-2 lg:grid-cols-[1fr_1.2fr] max-lg:grid-cols-1">
      ...
      <div className="overflow-hidden rounded-lg border border-transparent bg-[#1b1b2d]">
        ...
        {detail && (
          <div>
            <div className="border-b border-b-[#40415d] bg-[#28293a] px-3 py-1.5 text-xs font-extrabold tracking-wider text-(--sb-accent)">
              MAIN
            </div>
            {detail.main.map((cat) => (
              <OddsCategory
