    ...
    <>
      <section className="sb-card animate-deposit-panel flex min-h-[340px] flex-col overflow-hidden rounded-[1.15rem] backdrop-blur-sm">
            ...
            <button
              ...
              className={`relative mx-0.5 flex-1 cursor-pointer rounded-xl border py-2 text-[11px] font-bold transition-all duration-200 ${
                activeSlip === tab.id
                  ...
                  : "border-transparent bg-(--sb-bg-2)/70 text-(--sb-text-muted) hover:bg-(--sb-bg-2)"
              }`}
            >
              {tab.label}
              {showTabBadge ? (
                <span
                  className={`absolute -right-0.5 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-extrabold leading-none ${
                    activeSlip === tab.id
                      ? "bg-(--sb-bg-0) text-(--sb-accent-fill)"
        ...
        <button
          ...
          className="mt-auto flex w-full cursor-pointer items-center justify-center rounded-b-[1.1rem] border-0 bg-(--sb-accent-fill) py-3.5 text-base font-extrabold tracking-wide text-(--sb-on-accent) transition-all hover:bg-(--sb-accent-fill-hover) disabled:pointer-events-none disabled:opacity-50"
        >
      ...
      {/* Guest coupon confirmation modal */}
      {placedBet &&
