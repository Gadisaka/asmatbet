...
import { topHeaderData, sportsList } from "../data/homepageData";
...
function withLiveOdds(rows, liveOddsMap) {
  return rows.map((fx) => {
    const liveOdds = liveOddsMap.get(normalizeApiFixtureId(fx.api_fixture_id));
    return {
      ...fx,
      _liveElapsed: liveOdds?.elapsed ?? fx.elapsed ?? null,
      _liveElapsedSeconds: liveOdds?.elapsed_seconds ?? fx.elapsed_seconds ?? null,
      _liveStatus: liveOdds?.status ?? fx.live_status ?? null,
  ...
  return (
    <div className="border-t border-white/8 bg-gradient-to-b from-[#000000]/98 to-[#0a0a0a]/98">
      ...
      <div className="space-y-2 p-2.5">
        {showFilteredEmpty ? (
          <p className="rounded border border-[#293047] bg-[#0e1528] px-3 py-8 text-center text-xs text-(--sb-text-muted)">
  ...
  return (
    <button
      ...
      className={`flex h-[36px] w-full items-center justify-center rounded-xl border text-[13px] font-semibold transition-all duration-200 ${
        selected
          ? "border-(--sb-accent) bg-(--sb-accent-surface-deep) text-(--sb-accent-text-muted) shadow-[0_0_12px_rgba(143,207,74,0.35)]"
  ...
  return (
    <article
      className={`border-b border-white/8 transition-all duration-300 last:border-b-0 ${
        isExpanded
          ? "bg-[#0a0a0a]/60 ring-1 ring-(--sb-accent-fill)/45 shadow-[0_8px_28px_-8px_rgba(143,207,74,0.22)]"
          : "hover:bg-[#0a0a0a]/35"
      }`}
    >
      <div
      ...
      >
        <div className="grid grid-cols-[minmax(0,1fr)_72px_72px_72px_72px] items-center gap-2 max-md:grid-cols-1">
          <div className="min-w-0">
            <div className="mb-0.5 flex items-center truncate text-[11px] text-(--sb-text)">
...
function LiveMatchesList({
      ...
      <div className="flex flex-col items-center justify-center px-4 py-24 text-[rgba(255,255,255,0.72)]">
        <div className="relative h-14 w-14">
          ...
          <div className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#111111] ring-2 ring-red-500/30">
            <div className="h-7 w-7 animate-spin rounded-full border-2 border-[#8FCF4A] border-t-red-500" />
          </div>
        </div>
  ...
  return (
    <section className="sb-card mx-2 mb-3 mt-1 overflow-hidden rounded-[1.25rem] animate-deposit-panel">
      <header className="grid grid-cols-[minmax(0,1fr)_72px_72px_72px_72px] items-center gap-2 border-b border-white/8 bg-(--sb-bg-2)/40 px-3 py-2.5 text-[11px] font-semibold text-(--sb-text) max-md:grid-cols-1">
        <span>Teams</span>
...
function Live() {
  ...
  const refreshLive = useCallback(async () => {
    ...
    try {
      ...
      setLiveFixtures(withLiveOdds(apiRows, liveOddsMap));
    } catch (err) {
